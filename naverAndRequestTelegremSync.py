...
def send_telegram_links(query,chat_id):
     global old_links
     new_links = get_new_links(query,old_links)

     if new_links :
          send_telegram_message(chat_id,f'방금 업데이트 된 {query} 주제의 크롤링입니다.')
          for link in new_links:
               send_telegram_message(chat_id,link)
     old_links += new_links
...
if __name__ == '__main__':
    ...
    for query in queries:
        send_telegram_message(chat_id, f"{query}를 주제로 뉴스 기사 크롤링이 시작되었습니다.")
        old_links = [] #기존 링크드 리스트 초기화
        schedule.every(10).seconds.do(send_telegram_links, query=query,chat_id=chat_id)  # 주기적 실행 설정
